    ...
    for idx, app_id in enumerate(app_names[:limit], 1):
        ...
        try:
            ...
            task_stage_output = {
                ...
                "shuffle_data_summary": (lambda sd: {
                    "total_shuffle_read_gb": sd[0], "total_shuffle_write_gb": sd[1],
                    "total_disk_spill_gb": sd[2],
                    "max_stage_shuffle_read_gb": sd[3], "max_stage_shuffle_write_gb": sd[4],
                    "max_stage_shuffle_read_write_gb": sd[5],
                    "total_intermediate_data_gb": round(sd[1] + sd[2], 2),
                    "emr_serverless_storage_eligible": sd[1] + sd[2] <= 200,
                    "emr_serverless_storage_limit_gb": 200,
                })(
                    (lambda stages: (
                        round(sum(s.get("shuffle_read_gb") or 0 for s in stages), 2),
                        round(sum(s.get("shuffle_write_gb") or 0 for s in stages), 2),
                        round(sum(s.get("disk_spill_gb") or 0 for s in stages), 2),
                        round(max((s.get("shuffle_read_gb") or 0 for s in stages), default=0), 2),
                        round(max((s.get("shuffle_write_gb") or 0 for s in stages), default=0), 2),
                        round(max(((s.get("shuffle_read_gb") or 0) + (s.get("shuffle_write_gb") or 0) for s in stages), default=0), 2),
                    ))(stage_details)
                ),
                "total_cost_factor": cost_factor,
