...
def generate_dual_recommendations(input_path: str, limit: int = 100,
    ...
    for _, row in df_with_data.iterrows():
        ...
        def build_spark_cfg(max_exec, min_exec, sp, executor_disk, vcpu_override=None, mem_override=None):
            ...
            # Broadcast threshold: balance between OOM risk and shuffle cost.
            # Hard limit: Spark caps broadcast at 8GB. Source values like 1GB can cause
            # the planner to attempt broadcasting tables that grow beyond 8GB at runtime.
            # Cap at 256MB: large enough to broadcast dimension tables (reducing shuffle)
            # but safe from the 8GB ceiling and OOM from concurrent broadcasts.
            if _should_disable_broadcast(stages_raw, _spark_config_raw, s_out_gb, d_mem):
                ...
                cfg["spark.sql.autoBroadcastJoinThreshold"] = "256m"
