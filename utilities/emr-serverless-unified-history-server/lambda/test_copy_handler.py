...
class TestS3NotificationEvents(unittest.TestCase):
    ...
    def test_error_processes_all_then_raises(self):
        """All records are processed (not short-circuited), then raises to trigger retry/DLQ."""
        ...
        with self.assertRaises(RuntimeError) as ctx:
            copy_handler.lambda_handler(event, None)
        self.assertIn("1/2 copies failed", str(ctx.exception))
        self.assertEqual(self.mock_s3.copy_object.call_count, 2)

    def test_malformed_record_raises(self):
        with self.assertRaises(RuntimeError):
            copy_handler.lambda_handler({"Records": [{"s3": {}}]}, None)
    ...
    def test_multipart_aborts_on_failure_then_raises(self):
        self.mock_s3.create_multipart_upload.return_value = {"UploadId": "u1"}
...
class TestEventBridgeEvents(unittest.TestCase):
    ...
    def test_eventbridge_malformed_detail_raises(self):
